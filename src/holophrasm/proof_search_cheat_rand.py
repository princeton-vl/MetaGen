...
def global_get_payout(tree):
    try:
        t = time.time()
        data = global_interface.get_payout(tree, global_context)
        global_problem.payout_time += time.time()-t
        global_problem.payout_cnt += 1
        return data
    except:
        print ('ERROR IN GET PAYOUT')
        print (tree)
        print('%s: %s' % ('test', traceback.format_exc()))
def global_apply_prop(tree, prop_name, step=None):
    try:
        global_problem.tree_size += tree.size()
        t = time.time()
        data = global_interface.apply_prop(tree, global_context, prop_name, n=BEAM_SIZE, step=None)
        global_problem.gen_time += time.time()-t
        global_problem.gen_cnt += 1
        return data
    except:
...
def global_props(tree):
    try:
        t = time.time()
        data = global_interface.props(tree, global_context)
        global_problem.pred_time += time.time()-t
        global_problem.pred_cnt += 1
        return data
    except:
...
class ProofTree:
    def  __init__(self, tree, context, prop, prior_statement, _type, use):
        proof_step = tree_parser.proof_step(tree, context, prop, [x.step for x in prior_statement])
        self.step = proof_step
        self.child = []
        self.prop = prop
        self.type = _type
        self.use = use # gt_subs*2+gt_prop 
        if _type == 'p':
            l = len(prop.hyps)
            for i in range(l):
                j = l-1-i
                if prop.hyps[j].type == 'e':
                    self.child.insert(0, prior_statement[j])
        return 
    def print (self, s):
        print (s+str(self.step.tree))
        for c in self.child:
            c.print('    '+s)

    def replace_values(self, replacement_dict):
        self.tree = self.step.tree.copy().replace_values(replacement_dict)
        self.unconstrained = [t.copy().replace_values(replacement_dict) for t in self.step.unconstrained]
        for c in self.child:
            c.replace_values(replacement_dict)

    def search_node(self, tree):
        if tree == self.tree:
            return self
        for c in self.child:
            node = c.search_node(tree)
            if node is not None:
                return node
        return None
...
def build_proof_tree(args, prop, lm):
    ...
    statement_stack = []
    index = 0 
    use = [args.gt_subs, args.gt_prop, args.gt_payout, args.rand_prop, args.rand_subs, args.rand_payout]
...
class TypeA:
    ...
    def __init__(self, tree, depth, proven=False,label=None, gt_node_old=None):
        self.dead = False
        if proven:
        ...
        gt_node = global_problem.proof_tree.search_node(tree)
        if gt_node is not None:
            print ('create gt node', tree)
        if global_problem.proof_tree is not None and global_problem.proof_tree.use[2]:
            if gt_node is not None:
                self.initial_payout = 1
            else:
                self.initial_payout = 0.1
        ...
        self.gt_node = gt_node #global_problem.proof_tree.search_node(tree)#gt_node # if this is a positive node
        #if self.gt_node is not None:
        #    print  ('create a gt_node', tree)
    ...
    def create_child(self, child_params, parent_trees=None):
        ...
        if self.gt_node is not None and self.gt_node.use[0] and self.gt_node.prop.label == label:
            #print ('attempt to create a child with the gt prop %s' % (label))
            #print ('gt', self.gt_node.step.unconstrained)
            #print ('gt_replaced', [t.copy().replace_values(global_context.replacement_dict) for t in self.gt_node.step.unconstrained])
            #lptrees = global_apply_prop(self.tree, label)
            #print ('generated')
            #for lp_new, trees in lptrees:
            #    print (lp_new, trees)
            prop = self.gt_node.prop
            fit = self.tree.fit(prop.tree, [f for f in prop.f])
            for i in range(len(prop.unconstrained_variables)):
                fit[prop.unconstrained_variables[i]] = self.gt_node.unconstrained[i]
            trees = [hyps.tree.copy().replace(fit) for hyps in prop.hyps if hyps.type == 'e']
            lptrees = [(0, trees)]
            #print ('cheating')
            #for lp_new, trees in lptrees:
            #    print (lp_new, trees)
        ...
        while child is None and len(lptrees)>0:
            ...
            if not any(t in parent_trees for t in trees):
                if self.gt_node is not None and self.gt_node.use[0] and self.gt_node.prop.label == label:
                    #print (trees)
                    #print (self.gt_node.child)
                    child = TypeB(trees, np.exp(lp), self, label, self.gt_node)
                else:
    ...
    def get_props(self):
        ...
        if self.gt_node is not None and self.gt_node.use[1]:
            #log_probs.fill(-10)
            true_idx = labels.index(self.gt_node.prop.label)
            log_probs[true_idx] = self.gt_node.use[3]
...
class TypeB:
    ...
    def __init__(self, child_trees, prob, parent, label, gt_node=None):
        self.parent = parent
        self.label = label
        self.proven = False
        ...
        if  gt_node is not None:
            assert len(child_trees) == len(gt_node.child)
            self.children  = [self.create_child(t, node) for t, node in zip(child_trees, gt_node.child)]
        else:
    ...
    def create_child(self, tree, node=None):
        # check if the child has already been proven.
        child = global_problem.tsp.search(tree)
        if child is None: 
            if node is not None and node.type=='p':
                child = TypeA(tree, self.parent.depth+1, gt_node_old=node)
            else:
                child = TypeA(tree, self.parent.depth+1)
        return child
...
class ProofSearcher:
    def __init__(self, args, prop, lm, tree=None, directory='searcher', timeout=None):
        ...
        self.payout_cnt = 0
        self.tree_size = 0.0
        self.proof_tree = build_proof_tree(args, prop, lm) if args.gt_subs or args.gt_prop else None
        self.proof_tree.print('')
        ...
        if global_interface is None:
            ...
            global_interface = ProofInterface(args, lm, directory=directory)
        global_context = lm.standardize_context(prop)
        global_problem = self
        self.context = global_context
        print (global_context.replacement_dict)
        self.proof_tree.replace_values(global_context.replacement_dict)
        ...
        # build the root node
        self.root = TypeA(tree, 0, gt_node_old=self.proof_tree)
    ...
    def proof_object(self):
        ...
        out.save(directory=self.directory+'/'+self.proofdir)
        return out
