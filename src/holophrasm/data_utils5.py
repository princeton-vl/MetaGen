...
class LanguageModel:
    def __init__(self, database, _old=False):
        ...
        self.all_f = {}
        for p in self.database.propositions_list:
            for k,f in p.f.items():
                if k not in self.all_f:
                    self.all_f[k] = f
        print ('find %d f in the database' % (len(self.all_f)))
        ...
        if _old:
            ...
            self.extra_wffs     = len([k for k in self.all_f if self.all_f[k].vclass == 'wff'])
            self.extra_classes  = len([k for k in self.all_f if self.all_f[k].vclass == 'class'])
            self.extra_sets     = len([k for k in self.all_f if self.all_f[k].vclass == 'set'])
        # hand code these in.
        #self.extra_sets = 20
        #self.extra_wffs = 18
        ...
        num_training = int(len(list_of_propositions)-num_test-num_validation)
        ...
        #self.max_unconstrained_arity = max([p.unconstrained_arity() for p in self.database.propositions.values()])
    ...
    def axiom_counts(self):
        ...
        # for p in self.database.non_entails_axioms.values():
        #     out[p.arity()]+=1
    ...
    def random_replacement_dict_f(self, f = None):
        ...
        wff_random = np.random.choice(self.extra_wffs, len(statement_wffs), replace=False if len(statement_wffs) <= self.extra_wffs else True) if self.extra_wffs>0 else {}
        class_random = np.random.choice(self.extra_classes, len(statement_classes), replace=False if len(statement_classes) <= self.extra_classes else True) if self.extra_classes>0 else {}
        set_random = np.random.choice(self.extra_sets, len(statement_sets), replace=False if len(statement_sets) <= self.extra_sets else True) if self.extra_sets>0 else {}
...
def prop_applies_to_statement(tree, prop, context, vclass=None):
    ...
    #context_variables = set(f.label for f in context.f.values())
    #prop_variables = set(f.label for f in prop.f.values())
    prop_variables = [f for f in prop.f]
...
labels = test_all_props(statement,database.propositions.values())
%timeit searchproblem.search(statement.tree,p)
%timeit test_all_props(statement,database.propositions.values())
...
class SearchProblem:
    def __init__(self,database, max_unconstrained_arity = None, labels = None):
        ...
        if labels is None:
            ...
            for l in labels:
                self.add(database.propositions[l])
    def add(self,p):
        current = self.start
        observed_positions = []
    ...
    def search_dictionary(self,tree,context, max_proposition = None, vclass=None):
        ...
        for l in restricted_labels:
            ...
            if fit is not None:
                out[l]=fit
        return out
    def tree_match_l(self, tree, f, current = None):
        if current == None: 
            current = self.start
        ...
        if type(tree) is str:
            out = []
            for k in dictionary:
                out += self.tree_match_l(tree, f, dictionary[k])
            return out
        value = tree.value_at_position(position)
        out = []
        if value in dictionary: out += self.tree_match_l(tree, f, current = dictionary[value])
        if value in f:
            for k in dictionary:
                out += self.tree_match_l(self.wildcard, f, dictionary[k])
        return out


class SearchProblem_new:
    #def __init__(self,database, max_unconstrained_arity = None, labels = None):
    ...
    def add(self, expr, idx):
        ...
        while True:
            position, dictionary = current
            observed_positions.append(position)
            value = expr.tree.value_at_position(position)
            if value in expr.f: value = self.wildcard #variables count as wildcards
            ...
            next_pair = dictionary[value]
            if type(next_pair) is list:
                next_pair.append(idx)
                if len(next_pair) > self.expand_threshold:
                    self.expand(current,observed_positions, value, expr)
                return
    ...
    # I use the fact that the degree of any wildcard node is 0
    def expand(self,current, observed_positions, value, expr):
        # expand the node at current[1][value]
        all_positions = expr.tree.breadth_first_position_list()
    ...
    def value_at_position(self, tree, f, position):
        current = tree
        if current.value in f:
            return 'under_var'
        for i in position:
    ...
    def tree_match_l(self, tree, f, var=[], current = None):
        if current == None:
            current = self.start
        if type(current) is list: 
            return current
        ...
        #        out += self.tree_match_l(tree, f, dictionary[k])
        #    return out
        value = self.value_at_position(tree, f, position)
        out = []
        if value == 'under_var':
            for k in dictionary:
                out += self.tree_match_l(tree, f, var, dictionary[k])
        if value in dictionary: 
            out += self.tree_match_l(tree, f, var, dictionary[value])
        #if value in f:
        #    for k in dictionary:
        #        out += self.tree_match_l(self.wildcard, f, dictionary[k])
        if value in var and self.wildcard in dictionary:
            out += self.tree_match_l(tree, f, var, dictionary[self.wildcard])
        return out

if __name__ == "__main__":
    searcher = SearchProblem(database)
    samples = random.sample(lm.database.propositions.keys(), 200)
    t1 = 0
    t2 = 0
    l1 = 0 
    l2 = 0
    for l in samples:
        f = lm.database.propositions[l].f
        tree = lm.database.propositions[l].tree
        t = time.time()
        #labels = searcher.tree_match(tree)
        #labels = searcher.tree_match_l(tree, f)
        labels = searcher.search(tree, lm.database.propositions[l])
        l1 += len(labels)
        t1 += time.time() - t
        t = time.time()
        labels_slow = []
        for k,p in lm.database.propositions.items():
            if tree.fit(p.tree, p.f) is not None:
            #if p.tree.fit(tree, f) is not None:
                labels_slow.append(k)
        l2 += len(labels_slow)
        t2 += time.time()-t
        print (t1, t2, l1, l2, l)
        for k in labels_slow:
            assert k in labels
