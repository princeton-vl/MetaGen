...
# we are allowed to use one new var in each type
PERMIT_NEW_CLASSES = True
PERMIT_NEW_WFFS = True
...
class BeamSearchInterface:
    ...
    def __init__(self, args, gen_model, gen_logs, gen_rewards):
        # load the variables
        ...
        self.gen_rewards = gen_rewards

    def instance(self, params):
        ''' in this case, params = tree, context, prop_name '''
        tree, context, prop_name,  allowed, return_replacement_dict, step = params
        ...
        return BeamSearchState(tree, context, prop,  self.config, self.args, self.gen_model, self.gen_logs, self.gen_rewards, allowed_constructors=allowed, return_replacement_dict=return_replacement_dict, gt_step=step)
...
class BeamSearchState:
    def __init__(self, tree, context, prop, config, args, gen_model, 
            gen_logs, gen_rewards, allowed_constructors=None, beam_search_state=None,
        ...
        self.gen_logs = gen_logs if beam_search_state is None else beam_search_state.gen_logs
        self.gen_rewards = gen_rewards if beam_search_state is None else beam_search_state.gen_rewards
        ...
        self.prob = None if beam_search_state is None else beam_search_state.prob
        ...
        if beam_search_state is None:
            self.fit = context.fit
            #data_utils.prop_applies_to_statement(tree, prop, context)
    ...
    #def standardize_context(self, tree, context_hyps, fit):
    ...
    def get_all_allowed_symbols(self, allowed_symbols):
        ...
        hyp_symbols = [s.set() for s in self.context.hyps]
    ...
    def prepare_inputs(self, tree, context, fit, this_ua, prop):
        ...
        uv_dict = {v:'UC' for l,v in context.uv_dict.items()}
        uv_dict['UC'+this_ua] = 'TARGET_UC'
        statement = self.tree.copy().replace_values(uv_dict) # change the name of unconstrained var

        hyps = [hyp.copy()
                for hyp in context.hyps]
    ...
    def copy(self):
        return BeamSearchState(None, None, None, None, None, None, None, None, beam_search_state=self)
    ...
    def step(self):
        ...
        left = [None] if self.left_sibling_stack[-1] is None else self.left_sibling_stack[-1]
        #parent = [None]*len(self.vs) if self.parent_stack[-1] is None else self.parent_stack[-1]
        parent = [None] if self.parent_stack[-1] is None else self.parent_stack[-1]
    ...
    def get_logits_and_next_state(self, left, parent, structure_data):
        ...
        token = torch.LongTensor([self.config.encode[self.next_symbol]]).to(self.args.device)
        tree = torch.Tensor(structure_data).to(self.args.device).view(1, 4)
        if self.args.train_rl:
            h, logits = self.gen_model.logits_and_next_state(self.h, token, tree)
            self.h = h
            self.logits = logits[:len(self.config.encode)]
            self.prob = self.logits.exp()
        else:
            with torch.no_grad():
                h, logits = self.gen_model.logits_and_next_state(self.h, token, tree)
            self.h = h
            self.logits = logits.cpu().numpy()[:len(self.config.encode)]
            ...
            #    self.logits[key_location] = logits[value_location]
            self.prob = np.exp(self.logits)
    ...
    def finalize(self):
        ...
        if self.return_replacement_dict:
            ...
            #print (self.fit)
            new_tree = self.prop.tree.copy().replace(self.fit)
            #reverse_uv = {v:k for k,v in self.context.uv_dict.items()}
            #new_tree = self.tree.copy().replace_values(reverse_uv).replace(self.fit) 
            reverse_replacement = {v:k for k,v in self.context.replacement_dict.items()}
            return self.value, new_tree.replace_values(reverse_replacement)

    def next_child_sample(self):
        if MAX_BEAM_SEARCH_LENGTH is not None and self.total_symbols >= MAX_BEAM_SEARCH_LENGTH:
            return None 
        if self.prob is None:
            return None
        idxes = list(range(len(self.prob)))
        if not self.args.train_rl:
        ...
        while True:
            if self.args.train_rl:
                ...
                if random.random() > self.args.epsilon:
                    ...
                    action = m.sample()
                self.gen_logs[-1].append(m.log_prob(action))
                idx = action.item()
            else:
                idx = random.choices(range(len(prob)), prob)[0]
            index = idxes[idx]
            ...
            if self.is_valid(symbol):
                ...
                if self.args.train_rl:
                    self.gen_rewards[-1].append(1)
                return out
            idxes = idxes[:idx]+idxes[idx+1:]
            if self.args.train_rl:
                self.gen_rewards[-1].append(None)
                prob = torch.cat((prob[:idx], prob[idx+1:]), 0)
            else:    
                prob = prob[:idx]+prob[idx+1:]
        return None
    ...
    def next_child(self, must_be_better_than):
        ...
        if self.gt_step is not None and self.returned_symbols == 0:
            symbol = self.config.decode[self.logit_order[self.returned_symbols]]
            print ('the first symbol', symbol)
            print ('expected symbol', self.gt_string[len(self.string)])
            print ('is valid', self.is_valid(symbol))
    ...
    def generate_disallowed_symbols_list(self):
        ...
        self.disallowed_symbols = set(self.config.p.special_symbols).union(distincts)
