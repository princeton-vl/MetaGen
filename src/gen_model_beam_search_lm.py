...
class BeamSearchInterface:
    ...
    def __init__(self, args, gen_model):
        # load the variables
    ...
    def instance(self, params):
        ...
        return BeamSearchState(tree, context, prop,  self.config, self.args, self.gen_model, allowed_constructors=allowed, return_replacement_dict=return_replacement_dict, gt_step=step)
...
class BeamSearchState:
    ...
    def get_logits_and_next_state(self, left, parent, structure_data):
        ...
        if self.args.random_generate:
            self.h = None
            self.logits = np.ones(len(self.config.encode))
        else:
