...
class BeamSearchInterface:
    ...
    def __init__(self, vs, args, gen_model, config=None):
        # load the variables
    ...
    def instance(self, params):
        ...
        return BeamSearchState(tree, context, prop, self.config, self.vs, self.args, self.gen_model, allowed_constructors=allowed, return_replacement_dict=return_replacement_dict, gt_step=step)
...
class BeamSearchState:
    ...
    def prepare_inputs(self, tree, context, fit, this_ua, prop, separate=False):
        ...
        if separate:
            ...
            return known_tokens, known_trees, to_prove_tokens, to_prove_trees
        in_string, structured_data = data_utils_new.merge_graph_structures_new([known_graph_structure, to_prove_graph_structure]) 
        tokens = [self.config.encode[t] for t in in_string]
        return tokens, structured_data
    
    def extract_from_graph(self, graph):
        string = graph.string
        string = [self.config.encode[t] for t in string]
    ...
    def set_up_next_model(self):
        ...
        #in_tokens, structured_data = self.prepare_inputs(self.tree, self.context, self.fit_initial, self.this_ua, self.prop)
        ...
        known_tokens, known_trees, to_prove_tokens, to_prove_trees = self.prepare_inputs(self.tree, self.context, self.fit_initial, self.this_ua, self.prop, separate=True)   
        
        
        known_tokens = [torch.LongTensor(known_tokens).to(self.args.device)]
        to_prove_tokens = [torch.LongTensor(to_prove_tokens).to(self.args.device)]
        known_trees = [torch.Tensor(known_trees).to(self.args.device)]
        to_prove_trees = [torch.Tensor(to_prove_trees).to(self.args.device)]
        #print ('in_forward', known_tokens[0].shape, known_trees[0].shape, to_prove_tokens[0].shape, to_prove_trees[0].shape)
        #self.request = [torch.Tensor([1]), known_tokens, known_trees, to_prove_tokens, to_prove_trees]
        #self.h = self.send_request()
        if self.args.stat_model:
            h = torch.zeros(1,1,self.gen_model.gru.hidden_size).to(self.args.device)
        else:
            with torch.no_grad():
                h = self.gen_model.in_forward([known_tokens, known_trees, to_prove_tokens, to_prove_trees, None, None])
                h = self.gen_model.decoder.to_middle(h)
        #print (h.shape)
        self.h = h
    ...
    def send_request(self):
        ...
        for t in self.request:
            dist.send(tensor=t, dst=0)
        dist.recv(tensor=h, src=0)
        if self.request[0].item() == 2:
            dist.recv(tensor=logits, src=0)
            return h, logits
        return h

    def copy(self):
        return BeamSearchState(None, None, None, None, None, None, None, beam_search_state=self)
    ...
    def get_logits_and_next_state(self, left, parent, structure_data):
        ...
        #print ('logits', self.h.shape, token.shape, tree.shape)
        #self.request = [torch.Tensor([2]), self.h, token, tree]
        with torch.no_grad():
            h, logits = self.gen_model.logits_and_next_state(self.h, token, tree)
        #h, logits = self.send_request()
